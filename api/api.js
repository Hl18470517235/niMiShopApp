...
{
  return request.post("user/selectAdvertInfo",{},{ noAuth : true, region: true});
}

export function getRegionId(id)
{
  return request.post("user/findByIdAndName", {regionId: id}, {noAuth: true});
}

export function findNoticeList(){
  return request.post("user/selectAdvertInfo",{},{ noAuth : true, region: true});
}

export function findTimeLimitProduct(data)
{
  data.isFlag = 0;
  return request.post("product/manager/findTimeLimitProduct", data, { noAuth : true, region: true});
}

export function findRecommendProduct(data)
{
  data.isFlag = 0;
  return request.post("product/user/findRecommendProduct", data, { noAuth : true, region: true});
}

export function findProtocol(title) {
  return request.post('webmanage/searchProtocol', {protocolMainTitle: title}, { noAuth: true });
}
...
export function registerVerify(phone, reset){
  return request.post('user/sendMsm', { phone: phone, type: reset === undefined ? 'reset' : reset },{noAuth:true})
}
