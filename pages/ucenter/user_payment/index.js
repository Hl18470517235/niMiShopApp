       ...
       success(res){
         if (res.confirm){
           rechargeRoutine({ price: value, type: 1 }).then(res => {
             ...
             return app.Tips({ title: '转入成功', icon: 'success' }, {tab:5,url:'/pages/ucenter/user_money/index'});
           }).catch(err => {
      ...
      rechargeRoutine({ price: value, type: 0}).then(res=>{
        ...
        wx.requestPayment({
          ...
          success: function (res) {
            that.setData({ 'userinfo.now_money': app.help().Add(value, that.data.userinfo.now_money) });
            return app.Tips({ title: '支付成功', icon: 'success' }, {tab:5,url:'/pages/ucenter/user_money/index'});
          },
          fail: function () {
